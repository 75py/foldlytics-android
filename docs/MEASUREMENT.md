...
Android configuration-change events contain only changed fields. A zero width, height, smallest width, orientation, or density means that field is absent from the delta, so Foldlytics merges the delta into the preceding configuration before classifying it. A configuration change with no display dimensions does not by itself turn a known posture into unknown. A checkpoint replaces the configuration rather than acting as a delta. An event with no configuration object, device startup or shutdown, or a collection interruption clears the reconstruction baseline; only subsequent evidence can establish it again. Diagnostic logs retain the original delta received from Android.
...
The cache retains session boundaries, inner active time, and its existing singleton package totals. For the three longest complete sessions with positive inner active time, Foldlytics replays stored raw events in the data layer with the existing session analyzer. Historical activity evidence, configuration deltas, posture/device-state checkpoints, and collection gaps seed this replay. Only these selected sessions retain combination totals; the diagnostic analysis shares the same historical read. Cache refresh, collection history, raw reads, and replay run inside one Room transaction so concurrent collection cannot mix revisions. No combination data is added to the database schema.

Each eligible interval is assigned once to the unordered set of distinct, definitely resumed packages. For example, Chrome alone for 5 minutes, Chrome and YouTube together for 10 minutes, and YouTube alone for 5 minutes yields three exclusive entries totaling 20 minutes. Same-package activities collapse to one package. Packages supported only by unresolved historical evidence do not join combinations or block attribution to definite packages. Android does not expose stable activity instance ids, so a genuinely split interval can still be approximated as a singleton if the other app has only ambiguous evidence.

The three largest entries are shown in descending duration, with deterministic label/package ordering for ties; combinations carry a localized simultaneous-use label. Non-launchable apps, any combination containing a non-launchable app, fourth and later entries, and intervals with no definite app belong to Other. A hidden member is never removed to turn a combination into a misleading singleton. If replay cannot reproduce the cached session identity, closing boundary, and active duration, its entire duration is Other. Displayed entries plus Other always equal the cached inner active time; Other is hidden when zero. Cached boundaries, duration statistics, and session ordering remain authoritative. Ties between sessions use newer start time and then the recorded event sequence.
...
Foldlytics counts an app's display time while its activity is definitely `ACTIVITY_RESUMED`, the screen is interactive, and the device is unlocked. Android's public usage events do not expose a stable activity instance id, so same-package, same-class resume, pause, and stop events are treated conservatively. A new resume makes that package assignable again while it is certainly visible, but it does not erase older unresolved same-class evidence; after a later pause or stop could have applied to either instance, time is not assigned to that package again until another resume proves current visibility or a device restart, shutdown, or collection-gap reset clears the old evidence. This app-assignment uncertainty does not make the display posture unknown, does not reduce data coverage, and does not stop known unrelated packages from receiving their own ranking time.

An app's cover and inner shares use only its classified display time: `cover / (cover + inner)` and `inner / (cover + inner)`. Time when the display is unknown is not assigned to either display; it is shown separately and excluded from totals and percentages. The By display view compares each app's outer and inner usage times. An app appears in Mostly outer or Mostly inner only when it was used longer on that display. Apps with equal time on both displays appear in neither list. Within each list, apps are ranked by time on that display rather than by percentage, so a brief 100% use does not outrank a longer use.

In split screen and other multi-resume situations, more than one app can be `ACTIVITY_RESUMED` at the same time. The display-specific app rankings can therefore have app totals that exceed device use time. The per-session breakdown assigns such intervals to an exclusive simultaneous-app combination, so its entries plus Other equal the session's inner active time. Both the rankings and session breakdown show only apps that can be launched from the device launcher.
...
## Repeatable session replay performance check

`StoredAnalysisRoomTest.measuresYearOfDenseHistoryWithThreeSelectedSessionReplays` creates an isolated file-backed Room database (`inner-session-performance-test.db`) with 365 synthetic days and 77,380 events. It closes and reopens Room after saving the fixture and deletes the dedicated test database in teardown. It measures one cold rebuild/load and three warm cached loads for the 365-day period, including persisted-history queries, diagnostic analysis, and selected-session replay. “Cold” means an empty aggregate cache and a fresh Room connection; OS page caches are not flushed. Results are emitted to the `InnerSessionPerformance` logcat tag as `coldMs` and `warmMs`; compare on the same emulator/device and build configuration. The test also checks that exactly three details are shown and each reconciles to its cached duration. These fixtures contain no user usage history.
