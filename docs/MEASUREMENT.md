...
Each session also retains inner active time by package. When exactly one distinct package is definitely resumed during an eligible interval, that package receives the interval's time. A package that is only possibly resumed because of older unresolved activity evidence does not block that attribution. Intervals with no definitely resumed package or multiple distinct definitely resumed packages are kept as unallocated time. Android does not provide a stable activity instance id, so this rule can approximate a genuinely split interval as the one definitely resumed app when the other app's evidence remains ambiguous. For the three longest complete sessions with positive inner active time, Foldlytics shows the start time, inner active time, the top three launchable apps by time, and the remaining time as Other. The Other row is shown only when its remaining time is positive. Non-launchable apps, fourth and later apps, and unallocated time are included in Other. Ties between sessions are ordered by newer start time and then by the recorded event sequence.
