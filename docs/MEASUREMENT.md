...
Foldlytics counts an app's display time while its activity is definitely `ACTIVITY_RESUMED`, the screen is interactive, and the device is unlocked. Android's public usage events do not expose a stable activity instance id, so two same-package, same-class resume events are treated conservatively: the package remains assignable while it is certainly visible, but after a pause or stop makes the remaining instance count uncertain, later time is not assigned to that package until a new resume or reset evidence is observed. This app-assignment uncertainty does not make the display posture unknown, does not reduce data coverage, and does not stop known unrelated packages from receiving their own ranking time.
