...
なら正当な0秒として残します。長く使った最大3回には、アプリまたは同時利用したアプリの
組み合わせを最大3件表示します。同時利用は確実にresume中のアプリを組み合わせ、各区間の
時間を1回だけ数えます。上位3件以外、ランチャーから起動できないアプリを含む時間、アプリを
特定できない時間は「その他」にまとめます。確実にresume中のアプリが1つだけなら、過去のアクティビティイベントから別の
アプリがresume中かもしれない場合でも、そのアプリへセッション時間を配分します。このため、
Androidの根拠が曖昧な実際の分割利用は近似される場合があります。分析にはすでに端末内へ保存
している利用状況イベントを使い、新しい権限や利用履歴
の自動送信は追加しません。派生キャッシュは保存済みの元イベントから再生成できます。

このブランチでは、Gemini Nanoが利用可能な場合に、ML Kit Prompt APIでホームの説明文を自動生成します。
利用状況の入力と生成文は端末内で処理しますが、ML KitはAPIの性能・利用状況の指標をGoogleへ送信します。
Foldlyticsからモデルのダウンロードは開始しません。実機検証とプライバシーポリシー・Data safetyの
改定公開は未完了です。[実装方針とリリース条件](docs/MLKIT-INSIGHTS.md)を参照してください。
...
ホームアプリのウィジェット一覧からFoldlyticsを追加します。2×2から内側利用割合を円グラフで
表示し、文字サイズを大きくしても円グラフを維持します。横に広げると共有画像のように、
円グラフの横に内側・外側の利用時間と「開いた回数」をまとめて表示します。
縦に広げても各項目の間隔は変わりません。
期間は1日・7日・30日から選べ、初期値は7日です。

表示は最後に同期できた記録に基づきます。対象日付と最終同期日時を確認してください。
更新ボタンから同期でき、ウィジェット本体をタップすると対応する期間の分析を開きます。
記録がない場合や利用状況へのアクセスが必要な場合は、その状態を表示します。
