...
ホームの「あなたの使い方」は、昨日までの30日間の保存済み記録を短い文章で説明します。
十分な記録がある場合に自動で生成し、根拠となる数字も確認できます。
固定モデルを端末内で実行し、利用記録・生成文章・診断情報の外部送信は行いません。
モデルはインストール時に同梱し、アプリ利用中のダウンロードはありません。
モデル資産と初回展開分で約1.3GBの保存領域を使います。
ビルド方法と未検証事項は[実装・検証ガイド](docs/LOCAL-INSIGHTS.md)を参照してください。
