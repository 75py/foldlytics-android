...
にまとめます。分析にはすでに端末内へ保存している利用状況イベントを使い、新しい権限やデータ
の自動送信は追加しません。派生キャッシュは保存済みの元イベントから再生成できます。

アプリ別ランキングは、表示時間の合計を初期表示にし、外側・内側へ切り替えられます。
画面割合の表示では、判定時間のうち外側または内側の割合が高いアプリに分け、それぞれ
該当画面での表示時間が長い順に並べます。画面を判定できなかった時間は別に表示し、割合には
含めません。
