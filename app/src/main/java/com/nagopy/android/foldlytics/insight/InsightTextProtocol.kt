...
internal object InsightTextProtocol {
    ...
    fun systemPrompt(language: String): String = if (language == "ja") {
        ...
        Preserve the classified device usage denominator. A median is not an average.
    ...
    fun parse(output: String, facts: UsageInsightFacts): List<InsightSentence>? {
        ...
        val sentences = lines.map { line ->
            ...
            val allowedNumbers = numbers.findAll(fact.displayText)
    ...
    fun key(evidence: InsightEvidence, modelIdentity: String): String = sha256(
        "${VERSION}|$modelIdentity|${evidence.calibrationKey}|" +
