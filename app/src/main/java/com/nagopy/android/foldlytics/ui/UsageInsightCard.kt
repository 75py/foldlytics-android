...
internal fun UsageInsightCard(state: UsageInsightUiState) {
    ...
    LabCard(
        ...
        when (state.status) {
            ...
            InsightStatus.CHECKING, InsightStatus.GENERATING -> {
                LinearProgressIndicator(Modifier.fillMaxWidth())
                Text(stringResource(R.string.insight_generating))
            }
        ...
        Text(stringResource(R.string.insight_privacy_note), style = MaterialTheme.typography.bodySmall)
    }
    if (showEvidence) {
        AlertDialog(
