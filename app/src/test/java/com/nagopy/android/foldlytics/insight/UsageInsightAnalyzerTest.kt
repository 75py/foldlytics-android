...
class UsageInsightAnalyzerTest {
    ...
    @Test
    fun ranksAppsByInnerTimeAndUsesEachAppsOwnDenominator() {
        val result = analyze(
            rows(30),
            apps = listOf(
                app("other", inner = 10 * HOUR, cover = 0),
                app("first", inner = 100 * HOUR, cover = 300 * HOUR),
                app("hidden", inner = 500 * HOUR, cover = 0).copy(isLauncherApp = false),
            ),
        )
        val first = result.fact("inner_app_1").evidenceText
        assertTrue(first.contains("app_label_untrusted=\"first\""))
        assertTrue(first.contains("inner_percent_of_this_app_classified_time=25.0"))
        assertTrue(first.contains("not shares of device time"))
        assertFalse(result.promptEvidence.contains("hidden"))
    }

    @Test
    fun comparesAdjacentPeriodsInPercentagePointsOnlyWhenBothHaveGoodEvidence() {
        val previous = (31..60).map { day(today.minusDays(it.toLong()), inner = 2 * HOUR, cover = 2 * HOUR) }
        val result = analyze(rows(30) + previous)
        assertTrue(result.fact("previous_period_comparison").evidenceText.contains("change_percentage_points=-25.0"))
        assertFalse(analyze(rows(30) + previous.take(23)).facts.any { it.id == "previous_period_comparison" })
        val gaps = previous.mapIndexed { index, row -> row.copy(evidenceGapCount = if (index == 0) 1 else 0) }
        assertFalse(analyze(rows(30) + gaps).facts.any { it.id == "previous_period_comparison" })
    }
    ...
    private fun analyze(
        ...
        recordingStart: Long? = range.previousStartMillis,
        language: String = "en",
    ) = analyzer.analyze(source, apps, sessions, syncedThrough, now, zone, language, recordingStart)
    ...
    private fun app(label: String, inner: Long, cover: Long) = AppUsage(label, label, cover, inner, 0)
